...
{
    return type == TOK_LPAR || type == TOK_RPAR;
}
...
bool is_valid_arithmetic_syntax(int type1, int type2)
{
    ...
    if (type2 == TOK_UNKNOWN)
        return false;
    if (type1 != TOK_NUMBER && type2 != TOK_NUMBER)
    {
        if ((type1 == TOK_LPAR && type2 == TOK_LPAR) || (type2 == TOK_RPAR && type1 == TOK_RPAR))
            return true;
        if ((type1 == TOK_LPAR && type2 == TOK_RPAR) || (type2 == TOK_LPAR && type1 == TOK_RPAR))
            return false;
