...
void print_node_list(struct node_list *ast, FILE *f)
{
    ...
    if (c->next_sibling)
    {
        ...
        print_node_and_or(c->and_or, f, cast_to_void(c));
    }
    else
    ...
    while (c->next_sibling)
    {
        if (c->next_sibling->next_sibling)
        {
            ...
            print_node_and_or(c->next_sibling->and_or, f,
                cast_to_void(c->next_sibling));
        }
        else
            print_node_and_or(c->next_sibling->and_or, f, cast_to_void(c));
        c = c->next_sibling;
        // print_node_and_or(c->next_sibling->and_or, f, cast_to_void(c));
        // c = c->next_sibling;
        // if (c->type == SEMI)
...
void print_node_and_or(struct node_and_or *ast, FILE *f, void *node)
{
    ...
    if (ast->right)
    {
        ...
        print_node_pipeline(ast->right, f, cast_to_void(ast));

        if (ast->is_final)
            print_node_pipeline(ast->left.pipeline, f, cast_to_void(ast));
        else
            print_node_and_or(ast->left.and_or, f, cast_to_void(ast));
    }
    else
    {
        if (ast->is_final)
            print_node_pipeline(ast->left.pipeline, f, cast_to_void(node));
        else
            print_node_and_or(ast->left.and_or, f, cast_to_void(node));
    }
}
...
void print_node_pipeline(struct node_pipeline *ast, FILE *f, void *node)
{
    ...
    if (ast->is_not)
    {
        ...
        //     fprintf(f, "\tnode_%p -> node_%p;\n", cast_to_void(ast, cast_to_void(c));
        //     print_node_command(c->command, f, cast_to_void( c));
        // }
        // else
        print_node_command(c->command, f, cast_to_void( ast));
    }
    else
    {
        if (c->next_sibling)
    ...
    {
        PRINT_NODE("PIPE");
        //fprintf(f, "\tnode_%p -> node_%p;\n", cast_to_void(ast, cast_to_void(c->next_sibling->command));
        c = c->next_sibling;
        print_node_command(c->command, f, cast_to_void(ast));
    }
}
    ...
    void *node)
{
    ...
    if ((p && p->next) || (e && p))
    {
        ...
        while (p)
        {
            print_node_prefix(p, f, cast_to_void(ast));
            p = p->next;
        }
        if (e) /* link the root node to the first element (ex: echo) */
        {
            print_node_element(e, f, cast_to_void(ast));
...
void print_node_funcdec(struct node_funcdec *ast, FILE *f, void *node)
{
    ...
    fprintf(f, "\tnode_%p -> node_%p;\n", node, cast_to_void(ast));
    fprintf(f, "\tnode_%p [label=\"%s\"];\n", cast_to_void(ast->function_name),
        ast->function_name);
    fprintf(f, "\tnode_%p -> node_%p;\n", cast_to_void(ast),
        cast_to_void(ast->function_name));
    ...
    PRINT_NODE("(");
    PRINT_NODE(")");
    print_node_shell_command(ast->shell_command, f, cast_to_void(ast));
}
...
void print_node_redirection(struct node_redirection *ast, FILE *f, void *node)
{
    ...
    fprintf(f, "\tnode_%p [label=\"%s\"];\n", cast_to_void(ast), redirection);
    fprintf(f, "\tnode_%p -> node_%p;\n", node, cast_to_void(ast));
    if (ast->left && !is(ast->left, ""))
    {
        fprintf(f, "\tnode_%p [label=\"%s\"];\n", cast_to_void(ast->left),
            ast->left);
        fprintf(f, "\tnode_%p -> node_%p;\n", cast_to_void(ast), cast_to_void(ast->left));
        PRINT_NODE(ast->left);
    }
    if (ast->right && !is(ast->right, ""))
    {
        fprintf(f, "\tnode_%p [label=\"%s\"];\n", cast_to_void(ast->right),
            ast->right);
        fprintf(f, "\tnode_%p -> node_%p;\n", cast_to_void(ast), cast_to_void(ast->right));
...
{   //while a do echo toto done
    ...
    if (node)
        fprintf(f, "\tnode_%p -> node_%p;\n", node, cast_to_void(ast));
    print_node_compound_list(ast->condition, f, cast_to_void(ast));
    print_node_do_group(ast->body, f, cast_to_void(ast));
...
void print_node_case(struct node_case *ast, FILE *f, void *node)
{
    ...
    //printf("%s\n", ast->word);
    PRINT_NODE("IN");
    fprintf(f, "\tnode_%p [label=\"CASE\n%s\"];\n", cast_to_void(ast), ast->word);
    if (node)
        fprintf(f, "\tnode_%p -> node_%p;\n", node, cast_to_void(ast));
    if (ast->is_case_clause)
    {
        //PRINT_NODE("CASE CLAUSE");
        print_node_case_clause(ast->case_clause, f, cast_to_void(ast));
    }
    PRINT_NODE("ESAC");
}
...
void print_node_if(struct node_if *ast, FILE *f, void *node)
{
    ...
    print_node_compound_list(ast->condition, f, cast_to_void(ast));
    PRINT_NODE("THEN");
    print_node_compound_list(ast->if_body, f, cast_to_void(ast));
    if (ast->else_clause)
    {
        //PRINT_NODE("ELSE");
        // fprintf(f, "\tnode_%p [label=ELSE];\n", cast_to_void( ast->else_clause));
        // fprintf(f, "\tnode_%p -> node_%p;\n", cast_to_void(ast, cast_to_void( ast->else_clause));
        print_node_else_clause(ast->else_clause, f, cast_to_void(ast));
    }
    PRINT_NODE("FI");
}
...
void print_node_for(struct node_for *ast, FILE *f, void *node)
{
    ...
    if (node)
        fprintf(f, "\tnode_%p -> node_%p;\n", node, cast_to_void(ast));
    fprintf(f, "\tnode_%p [label=\"%s\"];\n", cast_to_void(ast->variable_name),
        ast->variable_name);
    fprintf(f, "\tnode_%p -> node_%p;\n", cast_to_void(ast), cast_to_void(ast->variable_name));
    struct range *r = ast->range;
    struct range *tmp = cast_to_void(ast);
    while (r)
    {
        fprintf(f, "\tnode_%p [label=\"%s\"];\n", cast_to_void(r), r->value);
        fprintf(f, "\tnode_%p -> node_%p;\n", cast_to_void(tmp), cast_to_void(r));
        tmp = r;
        PRINT_NODE(r->value);
        r = r->next;
    }
...
void print_node_else_clause(struct node_else_clause *ast, FILE *f, void *node)
{
    ...
    if (ast->type == ELSE)
    {
        ...
        if (node)
            fprintf(f, "\tnode_%p -> node_%p;\n", node, cast_to_void(ast));
        print_node_compound_list(ast->clause.else_body, f, cast_to_void(ast));
    }
...
void print_node_case_item(struct node_case_item *ast, FILE *f, void *node)
{
    ...
    if (ast && ast->words)
    {
        ...
        if (ast->compound_list)
            print_node_compound_list(ast->compound_list, f, cast_to_void(ast));
    }
